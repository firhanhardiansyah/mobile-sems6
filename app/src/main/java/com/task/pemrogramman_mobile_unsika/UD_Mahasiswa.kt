...
class UD_Mahasiswa : AppCompatActivity() {
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    var npm   = findViewById<EditText>(R.id.npm)
    var name  = findViewById<EditText>(R.id.nama)
    ...
    btnUpdate.setOnClickListener {
      ...
      val url       = "${base_url}mhs_update&mhs_id=${getMhsId}&npm=${npm.text}&nama=${name.text}&jenis_kelamin=${valueJenisKelamin}&prodi=${prodi.selectedItem}&hobi=${valueHobi}"
  ...
  fun delete(id: String) {
    ...
    val domain    = "https://hanzdeveloper.000webhostapp.com"
    val base_url  = "${domain}?params="
    val url       = "${base_url}mhs_delete&id=${id}"
